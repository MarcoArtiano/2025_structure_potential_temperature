...
import Trixi: varnames, cons2cons, cons2prim, cons2entropy, entropy, FluxLMARS, boundary_condition_slip_wall
...
@inline function flux_theta_sto(u_ll, u_rr, orientation::Integer,
    equations::CompressibleEulerPotentialTemperatureEquations2D)
    # Unpack left and right state
    rho_ll, v1_ll, v2_ll, p_ll = cons2prim(u_ll, equations)
    rho_rr, v1_rr, v2_rr, p_rr = cons2prim(u_rr, equations)
...
@inline function stolarsky_mean_opt(x::RealT, y::RealT, gamma::RealT, xg::RealT, yg::RealT, equations::CompressibleEulerPotentialTemperatureEquations2D) where {RealT<:Real}
    ...
    if f2 < epsilon_f2
        ...
        #=   t = @elapsed begin 
             test = (gamma - 1) / gamma * (yg - xg)*x*y / (yg*x - xg*y)
          end
          @info "Inside Optimized" t
          t = @elapsed begin 
             test = (gamma - 1) / gamma * (x + (y*yg - yg*x)/(y*yg - xg*y))
          end
          @info "new Inside Optimized" t
          t = @elapsed begin 
             test = (gamma - 1) / gamma * (y^gamma - x^gamma) /
         (y^(gamma - 1) - x^(gamma - 1))
          end
          @info "Inside not Optimized" t =#
