...
function plot_time_momentum(; cells_per_dimension = (200, 50), polydeg = 3, alfa, namenc = "EulerNC", name = "Euler", form = "false", xr_B = 40000)
	...
	lines!(ax1,  t4[:,2], z; label = L"$7 h$", kwargs_plot..., color = colors[4], linestyle = :dash)
	lines!(ax1,  t5[:,2], z; label = L"$8 h$", kwargs_plot..., color = colors[5], linestyle = :dashdot)

    lines!(ax2,  t1nc[:,2], znc; label = L"$2 h$", kwargs_plot..., color = colors[1])
	lines!(ax2,  t2nc[:,2], znc; label = L"$4 h$", kwargs_plot...,color = colors[2])
	lines!(ax2,  t3nc[:,2], znc; label = L"$6 h$", kwargs_plot...,color = colors[3])
	lines!(ax2,  t4nc[:,2], znc; label = L"$7 h$", kwargs_plot...,color = colors[4], linestyle = :dash)
	lines!(ax2,  t5nc[:,2], znc; label = L"$8 h$", kwargs_plot...,color = colors[5], linestyle = :dashdot)
	...
	save(pwd()*"/test_cases/linearnonhydrostatic/plots/linearnonhydrostatic_$(name).pdf", fig)

	return nothing
end


function plot_time_comparison(; cells_per_dimension = (200, 50), polydeg = 3, alfa, form = "false", xr_B = 40000)
    namenc = "EulerNC"
    ...
    kwargs_plot = (linewidth = 2.5,)

	# scatterlines!(ax1,  t5nce[:,2], z;marker=:diamond, label = L"$\varrho E$ NC", kwargs_plot..., color = colors[1])
    # scatterlines!(ax1, t5e[:,2], z; marker = :utriangle, color = colors[2], markersize = 10, label = L"$\varrho E$", kwargs_plot...)
    # scatterlines!(ax1,  t5ncp[:,2], z; marker =:diamond, label = L"$\varrho \theta$ NC", kwargs_plot..., color = colors[3])
    # scatterlines!(ax1,  t5p[:,2], z; label = L"$\varrho \theta$",  marker = :circle , markersize = 8, kwargs_plot..., color = colors[4])

    lines!(ax1,  t5nce[:,2], z; label = L"$\varrho E$ NC", kwargs_plot..., color = colors[1], linestyle = :solid)
    lines!(ax1, t5e[:,2], z; color = colors[2], label = L"$\varrho E$", kwargs_plot...)
    lines!(ax1,  t5ncp[:,2], z;  label = L"$\varrho \theta$ NC", kwargs_plot..., color = colors[1], linestyle = :solid)
    lines!(ax1,  t5p[:,2], z; label = L"$\varrho \theta$",  kwargs_plot..., color = colors[2])

    titlelegend = L"\text{Normalized momentum } m = \int_{-\infty}^{\infty}\overline{\varrho}(z)u'(x,z)w'(x,z)dx"
    leg = Legend(fig[1, 1], ax1, nothing , titlesize = 24, orientation = :horizontal, labelsize = 25.0)

	save(pwd()*"/test_cases/linearnonhydrostatic/plots/linearnonhydrostatic_comparison.pdf", fig)
...
function plot_time_comparison(; cells_per_dimension = (200, 50), polydeg = 3, alfa, form = "false", xr_B = 40000)
    ...
    titlesize = 30
	ax1 = Axis(fig[2, 1]; kwargs..., ylabel = L"$z$ [km]",ylabelsize = 25, titlesize = titlesize) 
...
function plot_time_comparison2(; cells_per_dimension = (200, 50), polydeg = 3, alfa, form = "false", xr_B = 40000)
    namenc = "EulerNC"
    t1nce,t2nce,t3nce,t4nce,t5nce,znce = retrieve_data_momentum(namenc, cells_per_dimension, polydeg, alfa, xr_B, form)
   ...
   titlesize = 28
	...
	save(pwd()*"/test_cases/linearnonhydrostatic/plots/linearnonhydrostatic_comparison_2.pdf", fig)
	return nothing
end


function retrieve_data_momentum(name, cells_per_dimension, polydeg, alfa, xr_B, form)
    folder_path = pwd()*"/test_cases/linearnonhydrostatic/out/"

    t1 = CSV.read(folder_path*"$(name)_2_$(cells_per_dimension[1])x$(cells_per_dimension[2])_$(polydeg)_$(alfa)_$(xr_B)_$(form).csv", DataFrame)
	t2 = CSV.read(folder_path*"$(name)_4_$(cells_per_dimension[1])x$(cells_per_dimension[2])_$(polydeg)_$(alfa)_$(xr_B)_$(form).csv", DataFrame)
	t3 = CSV.read(folder_path*"$(name)_6_$(cells_per_dimension[1])x$(cells_per_dimension[2])_$(polydeg)_$(alfa)_$(xr_B)_$(form).csv", DataFrame)
	t4 = CSV.read(folder_path*"$(name)_7_$(cells_per_dimension[1])x$(cells_per_dimension[2])_$(polydeg)_$(alfa)_$(xr_B)_$(form).csv", DataFrame)
	t5 = CSV.read(folder_path*"$(name)_8_$(cells_per_dimension[1])x$(cells_per_dimension[2])_$(polydeg)_$(alfa)_$(xr_B)_$(form).csv", DataFrame)
	z = t1[:,1]./1e3
...
plot_time_momentum(namenc = "PotentialNC", name = "Potential", alfa = 0.03)

plot_time_comparison(alfa=0.03)
plot_time_comparison2(alfa=0.03)